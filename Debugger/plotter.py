...
VEL_LIMIT_THETA = 20.0
ACC_LIMIT_THETA = 30.0

smoothing_window = 1  # Default smoothing window size
...
class PlotCanvas(FigureCanvasQTAgg):
    def __init__(self):
        ...
        super().__init__(fig)
        fig.tight_layout(pad=4.0, h_pad=3.5, w_pad=2.0)
...
def calculate_smoothed_data(time, theta, a, b, window=smoothing_window):
    ...
    Args:
        ...
        a: Array of alpha position values
        b: Array of beta position values
...
def update_plots(canvas, time, theta, a, b, window=smoothing_window, 
    ...
    Args:
        ...
        a: List or array of alpha position values
        b: List or array of beta position values
        window: Smoothing window size (default: 7)
    ...
    if show_raw:
        ax[0].plot(data['t'], data['theta'], 'b-', alpha=0.5)
    if show_smoothed:
        ax[0].plot(data['t'], data['theta_smooth'], 'r-', label='Smoothed')
    ax[0].set_title("Theta Position (deg)")
    ...
    if show_raw:
        ax[1].plot(data['t'], data['vtheta'], 'b-', alpha=0.5)
    if show_smoothed:
        ax[1].plot(data['t'], data['vtheta_smooth'], 'r-', label='Smoothed')
    ax[1].axhline(VEL_LIMIT_THETA, linestyle="--", color='gray')
    ax[1].axhline(-VEL_LIMIT_THETA, linestyle="--", color='gray')
    ax[1].set_title("Theta Velocity (deg/s)")
    ax[1].set_xlabel("Time (s)")
    ax[1].set_ylabel("Velocity (deg/s)")
    ax[1].legend()
    
    # alpha Position (left column, middle)
    if show_raw:
        ax[2].plot(data['t'], data['a'], 'b-', alpha=0.5)
    if show_smoothed:
        ax[2].plot(data['t'], data['a_smooth'], 'r-', label='Smoothed')
    ax[2].set_title("Alpha Position (deg)")
    ax[2].set_xlabel("Time (s)")
    ax[2].set_ylabel("Angle (deg)")
    ax[2].legend()
    
    # alpha Velocity (right column, middle)
    if show_raw:
        ax[3].plot(data['t'], data['va'], 'b-', alpha=0.5)
    if show_smoothed:
        ax[3].plot(data['t'], data['va_smooth'], 'r-', label='Smoothed')
    ax[3].axhline(VEL_LIMIT_A, linestyle="--", color='gray')
    ax[3].axhline(-VEL_LIMIT_A, linestyle="--", color='gray')
    ax[3].set_title("Alpha Velocity (deg/s)")
    ax[3].set_xlabel("Time (s)")
    ax[3].set_ylabel("Velocity (deg/s)")
    ax[3].legend()
    
    # beta Position (left column, bottom)
    if show_raw:
        ax[4].plot(data['t'], data['b'], 'b-', alpha=0.5)
    if show_smoothed:
        ax[4].plot(data['t'], data['b_smooth'], 'r-', label='Smoothed')
    ax[4].set_title("Beta Position (deg)")
    ax[4].set_xlabel("Time (s)")
    ax[4].set_ylabel("Angle (deg)")
    ax[4].legend()
    
    # beta Velocity (right column, bottom)
    if show_raw:
        ax[5].plot(data['t'], data['vb'], 'b-', alpha=0.5)
    if show_smoothed:
        ax[5].plot(data['t'], data['vb_smooth'], 'r-', label='Smoothed')
    ax[5].axhline(VEL_LIMIT_B, linestyle="--", color='gray')
    ax[5].axhline(-VEL_LIMIT_B, linestyle="--", color='gray')
    ax[5].set_title("Beta Velocity (deg/s)")
    ax[5].set_xlabel("Time (s)")
    ax[5].set_ylabel("Velocity (deg/s)")
    ax[5].legend()
